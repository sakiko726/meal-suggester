import streamlit as st
import pandas as pd
from datetime import datetime, timedelta

st.title("AIコーチ付き 食事 & 体調サポートアプリ 💛")

# =========================================================
# 初期化
# =========================================================
if "meals" not in st.session_state:
    st.session_state.meals = []

if "settings" not in st.session_state:
    st.session_state.settings = {
        "kcal_target": 1450,
        "protein_target": 100,
        "fat_target": 35,
        "carbs_target": 160,
        "coach_tone": "優しいお姉さん",
        "hormone_phase": "なし",
    }

s = st.session_state.settings


# =========================================================
# 🩸 生理周期チェック
# =========================================================
st.header("🩸 生理周期チェック")

period_start = st.date_input("直近の生理開始日を入力してね")
cycle_length = st.number_input("平均生理周期（日）", min_value=20, max_value=40, value=28)
period_length = st.number_input("生理期間の日数", min_value=3, max_value=10, value=5)

today = datetime.now().date()
phase = "不明"
phase_msg = ""

if period_start:
    days_passed = (today - period_start).days % cycle_length

    if days_passed < period_length:
        phase = "月経期"
        phase_msg = "今日はゆっくり休んでね…生きてるだけで偉いよ💛"

    elif days_passed < 14:
        phase = "卵胞期"
        phase_msg = "代謝が上がりやすい時期！ここから一緒にペース上げよ💛"

    elif days_passed == 14:
        phase = "排卵期"
        phase_msg = "魅力MAXの時期✨ 肌の調子も気分も良くなりやすいよ！"

    else:
        phase = "黄体期"
        phase_msg = "むくみ・食欲UPしがち！甘い物ほしくなるのはホルモンのせい🍫"

    st.subheader(f"📌 現在のあなたのフェーズ： **{phase}**")
    st.write(phase_msg)

    s["hormone_phase"] = phase


# =========================================================
# 1) 食材データ（カテゴリ付き）
# =========================================================
FOODS_BY_CATEGORY = {
    "主食": {
        "白ごはん": {"kcal": 168, "P": 2.5, "F": 0.3, "C": 37},
        "玄米": {"kcal": 165, "P": 2.8, "F": 1.0, "C": 34},
        "オートミール": {"kcal": 380, "P": 13.7, "F": 6.2, "C": 69},
        "食パン": {"kcal": 260, "P": 9, "F": 4, "C": 45},
        "うどん": {"kcal": 105, "P": 2.6, "F": 0.4, "C": 21},
        "そば": {"kcal": 120, "P": 4.8, "F": 1, "C": 24},
        "パスタ": {"kcal": 150, "P": 5, "F": 1, "C": 30},
    },
    "タンパク質": {
        "鶏むね肉": {"kcal": 165, "P": 31, "F": 4, "C": 0},
        "鶏ささみ": {"kcal": 105, "P": 24, "F": 0.8, "C": 0},
        "牛赤身": {"kcal": 182, "P": 21, "F": 10, "C": 0},
        "豚ロース": {"kcal": 240, "P": 19, "F": 17, "C": 0},
        "サーモン": {"kcal": 200, "P": 20, "F": 13, "C": 0},
        "ツナ缶（水煮）": {"kcal": 102, "P": 23.5, "F": 0.8, "C": 0},
        "卵": {"kcal": 151, "P": 12.3, "F": 10.3, "C": 0.7},
        "納豆": {"kcal": 200, "P": 16.5, "F": 10, "C": 12},
        "豆腐": {"kcal": 56, "P": 4.9, "F": 3, "C": 1.1},
    },
    "野菜": {
        "ブロッコリー": {"kcal": 33, "P": 4.3, "F": 0.5, "C": 7},
        "トマト": {"kcal": 18, "P": 0.7, "F": 0.1, "C": 3.8},
        "レタス": {"kcal": 15, "P": 1.0, "F": 0.2, "C": 2.8},
        "にんじん": {"kcal": 37, "P": 0.8, "F": 0.2, "C": 9},
        "ほうれん草": {"kcal": 20, "P": 2.1, "F": 0.4, "C": 3.1},
    },
    "果物": {
        "バナナ": {"kcal": 86, "P": 1.1, "F": 0.2, "C": 23},
        "りんご": {"kcal": 52, "P": 0.2, "F": 0.1, "C": 14},
        "いちご": {"kcal": 34, "P": 0.9, "F": 0.1, "C": 8},
        "みかん": {"kcal": 45, "P": 0.6, "F": 0.2, "C": 12},
    },
    "間食 / 脂質": {
        "ヨーグルト": {"kcal": 62, "P": 3.6, "F": 3, "C": 5.2},
        "チーズ": {"kcal": 356, "P": 22, "F": 29, "C": 2},
        "ナッツ": {"kcal": 600, "P": 20, "F": 50, "C": 20},
        "アボカド": {"kcal": 187, "P": 2.1, "F": 18, "C": 6},
        "チョコレート": {"kcal": 558, "P": 7, "F": 34, "C": 55},
        "アイス": {"kcal": 180, "P": 3, "F": 8, "C": 23},
        "クッキー": {"kcal": 490, "P": 6, "F": 23, "C": 66},
        "ケーキ": {"kcal": 430, "P": 5, "F": 24, "C": 50},
    }
}

# カテゴリ選択
category = st.selectbox("カテゴリを選ぶ", list(FOODS_BY_CATEGORY.keys()))

# 食材選択
selected_food = st.selectbox(
    "食べたものを選ぶ",
    list(FOODS_BY_CATEGORY[category].keys())
)


# =========================================================
# 🍽 食事入力
# =========================================================
st.header("🍽 食事入力（グラムでOK！）")

selected_food = st.selectbox("食べたものを選ぶ", food_names)
grams = st.number_input("食べた量（g）", min_value=1, max_value=2000, value=100)

if st.button("追加する 🍽️"):
    base = FOODS[selected_food]
    factor = grams / 100

    entry = {
        "time": datetime.now().strftime("%H:%M"),
        "food": selected_food,
        "grams": grams,
        "kcal": base["kcal"] * factor,
        "P": base["P"] * factor,
        "F": base["F"] * factor,
        "C": base["C"] * factor,
    }

    st.session_state.meals.append(entry)
    st.success("追加しました！✨")


# =========================================================
# 📘 今日の記録
# =========================================================
st.subheader("📘 今日の記録")

if len(st.session_state.meals) > 0:
    df = pd.DataFrame(st.session_state.meals)
    st.dataframe(df)

    total = df[["kcal", "P", "F", "C"]].sum()

    st.markdown("### 🍀 今日の合計")
    st.write(f"🔥 **kcal:** {total.kcal:.0f} / {s['kcal_target']}")
    st.write(f"💪 **Protein:** {total.P:.1f} / {s['protein_target']}")
    st.write(f"🥑 **Fat:** {total.F:.1f} / {s['fat_target']}")
    st.write(f"🍞 **Carbs:** {total.C:.1f} / {s['carbs_target']}")


# =========================================================
# 🎧 AIコーチのアドバイス
# =========================================================
if st.button("AIコーチのアドバイスを聞く 🎧"):

    tone = s["coach_tone"]
    hormone = s["hormone_phase"]

    df = pd.DataFrame(st.session_state.meals) if len(st.session_state.meals) else pd.DataFrame(columns=["kcal","P","F","C"])
    total = df[["kcal","P","F","C"]].sum()

    advice = ""

    # --- コーチ口調 ---
    if tone == "優しいお姉さん":
        advice += "💛 えらすぎる…今日も記録してくれて本当にかっこいいよ…！\n"
    elif tone == "厳しめトレーニー":
        advice += "💪 まだいけるぞ！追い込むぞ！タンパク質追加だ！\n"
    else:
        advice += "👓 ふむ、今日のデータを解析しますね。\n"

    # --- 栄養分析 ---
    if total.P < s["protein_target"] * 0.6:
        advice += "- 🥚 **タンパク質不足 → 甘い物欲しくなるよ！**\n"

    # --- ホルモン分析 ---
    if hormone == "黄体期":
        advice += "- 🍫 黄体期は食欲UPしやすい時期です。\n"
    if hormone == "月経期":
        advice += "- 🩸 鉄分とタンパク質を意識しよう！\n"

    # --- 次の食事提案 ---
    advice += "\n🍙 **次の食事はこれがおすすめ！**\n"
    if total.P < s["protein_target"]:
        advice += "- 鶏むね肉 / 卵 / ツナ缶 / ギリシャヨーグルト\n"
    if total.C < s["carbs_target"]:
        advice += "- さつまいも / オートミール / バナナ\n"
    if total.F < s["fat_target"]:
        advice += "- ナッツ / アボカド\n"

    st.info(advice)


# =========================================================
# ⚙️ サイドバー設定
# =========================================================
with st.sidebar:
    st.header("⚙️ 設定")

    s["kcal_target"] = st.number_input("目標 kcal", value=s["kcal_target"])
    s["protein_target"] = st.number_input("目標 P", value=s["protein_target"])
    s["fat_target"] = st.number_input("目標 F", value=s["fat_target"])
    s["carbs_target"] = st.number_input("目標 C", value=s["carbs_target"])

    st.markdown("### 🧑‍🏫 コーチ")
    s["coach_tone"] = st.selectbox("コーチの口調", ["優しいお姉さん", "厳しめトレーニー", "医者"])
